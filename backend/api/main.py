...
def _cookie_settings() -> Dict[str, Any]:
    """Return consistent cookie settings for auth/session cookies.

    - In local development (frontend on http://localhost/127.0.0.1), use
      SameSite=Lax and allow insecure cookies.
    - In all other cases (e.g. Vercel default domain, custom domains), use
      SameSite=None; Secure so that cross-site requests from the frontend can
      include the wf_session cookie.
    """
